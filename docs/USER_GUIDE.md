...
| 欄位 | 內容 |
|------|------|
| **關聯內規文件** | 文件名稱、版次、受影響章節 |
| **對應法規修正** | 法規條號、修正重點摘要 |
| **差異辨識** | 變更類型（新增/修正/刪除）與影響範圍 |
| **建議修正文** | AI 產出的具體修改文句 |
...
單次最多 50 份文件，每個檔案大小不超過 10MB。

### Q3：分析需要多久時間？

取決於文件數量與法規修正項目數量：

- 法規修正 10 條 + 內規 10 份：約 30-50 秒
- 法規修正 20 條 + 內規 50 份：約 60-90 秒
...
2. **版本管理**：檔名包含版本號（如 `內規_v3.7.docx`）
3. **完整性**：上傳完整的內規文件，避免僅上傳修訂部分

### 結果審核

1. **優先處理**：先檢視「新增」與「刪除」類型的建議
2. **交叉比對**：對照原始法規公告與內規文件
3. **法務確認**：重要條文修改應諮詢法務部門
4. **分階段採納**：可分批處理修改，降低風險
